...
class RandBox(nn.Module):
    def __init__(self, cfg):
        ...
        # Build Dynamic Head and pass text embeddings        
        ...
        kd_weight = cfg.MODEL.KD_WEIGHT
        aux_ce = cfg.MODEL.AUX_CE
        aux_nc_ce = cfg.MODEL.AUX_NC_CE
        # Build Criterion.
        matcher = HungarianMatcherDynamicK(
            cfg=cfg, cost_class=class_weight, cost_bbox=l1_weight, cost_giou=giou_weight
        )
        # weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_giou": giou_weight,
        #                "loss_nc_ce": nc_weight, "loss_decorr": decorr_weight, "loss_aux_ce": aux_ce,
        #                "loss_aux_nc_ce": aux_nc_ce, "loss_kd": kd_weight, }
        weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_giou": giou_weight,
                       "loss_nc_ce": nc_weight, "loss_decorr": decorr_weight, "loss_kd": kd_weight, }
    ...
    def model_predictions(self, backbone_feats, images_whwh, x, t, x_self_cond=None, clip_x_start=False, sample_i=0):
        ...
        outputs_class, outputs_objectness, outputs_coord, aux_cls_logits = self.head(backbone_feats, x_boxes, t, None)
        ...
        return ModelPrediction(pred_noise, x_start), outputs_class, outputs_objectness, outputs_coord, aux_cls_logits
        ...
        if self.sampling_method == 'Random':
            ...
            for sample_step in range(self.multiple_sample):
                for time, time_next in time_pairs:
                    ...
                    preds, outputs_class, outputs_objectness, outputs_coord, _ = self.model_predictions(backbone_feats, images_whwh, img,
                                                                                 time_cond,
                                                                                 self_cond, clip_x_start=clip_denoised,
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            t = t.squeeze(-1)
            x_boxes = x_boxes * images_whwh[:, None, :]
            # x_boxes是gt_instances 扩散后的框 噪声小就比较接近

            outputs_class, output_objectness, outputs_coord, aux_cls_logits = self.head(features, x_boxes, t, None)

            output = {'pred_logits': outputs_class[-1], 'pred_objectness': output_objectness[-1], 
                      'pred_boxes': outputs_coord[-1], 'aux_logits': aux_cls_logits[-1]}

            if self.deep_supervision:
                output['aux_outputs'] = [{'pred_logits': a, 'pred_objectness': b, 'pred_boxes': c, 'aux_logits':d}
                                         for a, b, c, d in zip(outputs_class[:-1], output_objectness[:-1], outputs_coord[:-1], aux_cls_logits[:-1])]
