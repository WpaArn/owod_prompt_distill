...
class DynamicHead(nn.Module):
    ...
    def forward(self, features, init_bboxes, t, init_features):
        ...
        for head_idx, rcnn_head in enumerate(self.head_series):
            class_logits, objectness, pred_bboxes, proposal_features, aux_cls_logits = rcnn_head(features, bboxes, proposal_features,
                                                                           self.box_pooler)
            ...
            if self.return_intermediate:
                ...
                inter_aux_cls_logits.append(aux_cls_logits)
            bboxes = pred_bboxes.detach()

        if self.return_intermediate:
            return torch.stack(inter_class_logits), torch.stack(inter_objectness), torch.stack(inter_pred_bboxes), torch.stack(inter_aux_cls_logits)

        return class_logits[None], objectness[None], pred_bboxes[None], aux_cls_logits[None]
...
class RCNNHead(nn.Module):
    ...
    def __init__(self, cfg, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        self.aux_predictor = AuxFastRCNNOutputLayers(cfg, 256, num_classes)
    
    def forward(self, features, bboxes, pro_features, pooler):
        """
        :param bboxes: (N, nr_boxes, 4)  gtins
        :param pro_features: (N, nr_boxes, d_model)
        """

        N, nr_boxes = bboxes.shape[:2]  # nr_boxes:500  N:
        ...
        # print(roi_features.shape)  #([N * nr_boxes, 256, 7, 7])
        ...
        # print(roi_features.shape)  # ([49, N * nr_boxes, 256])
        # print(pro_features.shape)  #([N, nr_boxes, 256])

        # self_att.    pro_features([N, nr_boxes, 256])
        pro_features = pro_features.view(N, nr_boxes, self.d_model).permute(1, 0, 2)
        pro_features2 = self.self_attn(pro_features, pro_features, value=pro_features)[0]
        pro_features = pro_features + self.dropout1(pro_features2)
        pro_features = self.norm1(pro_features)
        # print(pro_features.shape)   #([nr_boxes, N, 256])

        # inst_interact.
        pro_features = pro_features.view(nr_boxes, N, self.d_model).permute(1, 0, 2).reshape(1, N * nr_boxes, self.d_model)
        # print(pro_features.shape)  #([1, N * nr_boxes, 256])
        ...
        obj_features = self.norm3(obj_features)  # (1,1000,256)
        ...
        cls_clip_feature = fc_feature.clone()
        aux_pred_class_logits = self.aux_predictor(cls_clip_feature)

        class_logits = class_logits + aux_pred_class_logits

        bboxes_deltas = self.bboxes_delta(reg_feature)
        pred_bboxes = self.apply_deltas(bboxes_deltas, bboxes.view(-1, 4))

        return class_logits.view(N, nr_boxes, -1), objectness.view(N, nr_boxes, -1), pred_bboxes.view(N, nr_boxes, -1), obj_features, aux_pred_class_logits.view(N, nr_boxes, -1)
