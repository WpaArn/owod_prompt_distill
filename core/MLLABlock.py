...
class MLLABlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads=4, mlp_ratio=4., qkv_bias=True, drop=0.1, drop_path=0.,
...
if __name__ == '__main__':
    mlla_block = MLLABlock(dim=256, input_resolution=1500)

    batch_size = 1
    N = 1500
    input_tensor = torch.randn(batch_size, N, 256)  #([1, 1024, 64])

    output_tensor = mlla_block(input_tensor)
    print("Input shape:", input_tensor.shape)   #([1, 1024, 64])
    print("Output shape:", output_tensor.shape)  # ([1, 1024, 64])